    ...
    <div>
      ...
      {modal && (
        <Modal title={modal.isEdit ? (modal.form.car_type === 'other' ? t('car.title_edit_other') : t('car.title_edit_self')) : (modal.form.car_type === 'other' ? t('car.add_other') : t('car.add_self'))} onClose={() => setModal(null)} bodyClass="cm-modal-body">
          ...
          <input value={modal.form.model} onChange={(e) => setForm({ model: e.target.value })} maxLength={100} placeholder={t('car.model_placeholder')} className="form-input form-input--sm cm-field-mb" />
          <div className="cm-grid2">
            ...
            <div><label className="form-label">{t('car.seats_label')}</label><input type="number" min="1" max="40" value={modal.form.seats} onChange={(e) => setForm({ seats: e.target.value })} placeholder="0" className="form-input form-input--sm" /></div>
          </div>
          ...
          <input type="file" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" onChange={(e) => setForm({ imageFile: e.target.files[0] || null })} className="form-input form-input--sm cm-file-input" />
